...
from simulator.facility_topology import FacilityTopology, ZoneDefinition, available_presets, build_facility_topology
...
class SimulationEngine:
    ...
    def __init__(
        ...
        self.topology: FacilityTopology = build_facility_topology(preset_name)
        self.tick_minutes = tick_minutes
        self.live_tick_seconds = live_tick_seconds
        self.current_time = start_time or datetime(2026, 1, 5, 6, 0, tzinfo=timezone.utc)
        self._rng = random.Random(seed)
        self.occupancy_model = OccupancyModel()
        self.sensor_emulator = SensorEmulator()
        self.fault_injector = FaultInjector()
        init_db(self.database_path)
        self.repository = TelemetryRepository(create_session_factory(self.database_path))
    ...
    def run_tick(self, persist: bool = True) -> TickResult:
        ...
        active_faults = self.fault_injector.get_active_faults(self.current_time)
        occupancy_by_zone = {
            zone.zone_id: self._zone_occupancy(zone, active_faults)
    ...
    def run_live(self, ticks: int | None = None) -> list[TickResult]:
    ...
    def run_historical(
        ...
        return [self.run_tick(persist=persist) for _ in range(steps)]
    ...
    def _zone_occupancy(
        ...
        for fault in active_faults:
            if fault.fault_type == "occupancy_spike" and fault.applies_to_zone(zone.zone_id):
                occupancy += max(1, int(round(zone.capacity * 0.2 * fault.severity_factor)))
...
def generate_default_historical_data(
    ...
    csv_path = sample_csv_path or settings.sample_csv_path
    total_readings = 0
    sample_readings: list[SensorReading] = []
    batch: list[SensorReading] = []
    batch_row_target = 12_000

    for preset_index, preset_name in enumerate(available_presets()):
        start_time = datetime(2026, 1, 6 + preset_index, 0, 0, tzinfo=timezone.utc)
        ...
        for _ in range(steps):
            ...
            if len(sample_readings) < 500:
                remaining = 500 - len(sample_readings)
                sample_readings.extend(result.readings[:remaining])
            batch.extend(result.readings)
            if len(batch) >= batch_row_target:
                engine.repository.save_sensor_readings(batch)
                batch.clear()
    ...
    _export_sample_csv(csv_path, sample_readings)
...
def _schedule_default_faults(engine: SimulationEngine) -> None:
    ...
    flush_fixture = next(
        ...
        if item.fixture_type in {"flush_valve", "urinal"}
    )
    ...
    engine.fault_injector.inject_fault(
        ...
        severity="medium",
        start_time=engine.current_time + timedelta(days=1, hours=2),
    )
    engine.fault_injector.inject_fault(
        ...
        duration_minutes=90,
        severity="high",
        start_time=engine.current_time + timedelta(days=3, hours=1),
    )
    engine.fault_injector.inject_fault(
        ...
        severity="high",
        start_time=engine.current_time + timedelta(days=4, hours=5),
    )
    engine.fault_injector.inject_fault(
        ...
        duration_minutes=75,
        severity="medium",
        start_time=engine.current_time + timedelta(days=5, hours=3),
    )
    engine.fault_injector.inject_fault(
        ...
        duration_minutes=60,
        severity="medium",
        start_time=engine.current_time + timedelta(days=2, hours=8),
    )


def _export_sample_csv(path: Path, readings: list[SensorReading]) -> None:
...
def _build_cli_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="AquaSense AI Phase 1 simulator")
    subparsers = parser.add_subparsers(dest="command", required=True)

    historical = subparsers.add_parser("historical", help="Generate historical telemetry")
    historical.add_argument("--days", type=int, default=7)
    historical.add_argument("--seed", type=int, default=42)

    live = subparsers.add_parser("live", help="Run live telemetry simulation")
    live.add_argument("--facility", choices=available_presets(), default="airport")
    live.add_argument("--ticks", type=int, default=5)
...
def main() -> None:
    ...
    if args.command == "historical":
        ...
        print(
            f"Generated {summary.total_readings} readings into {summary.database_path} "
    ...
    engine = SimulationEngine(
        ...
        database_path=settings.database_path,
        start_time=datetime(2026, 1, 5, 6, 0, tzinfo=timezone.utc),
        tick_minutes=settings.tick_minutes,
        live_tick_seconds=settings.live_tick_seconds,
        seed=args.seed,
    )
    results = engine.run_live(ticks=args.ticks)
    for result in results:
        print(
            f"{result.timestamp.isoformat()} "
            f"occupancy={sum(result.occupancy_by_zone.values())} "
